...
### Z-score trending — why relative matters

Every other "trending" implementation I've seen uses absolute thresholds ("viral = 1M views"). That's useless for benchmarking competitors — a niche tech channel getting 50K views on a video might be massively outperforming their baseline.

VidMetrics Pulse measures each video against the **channel's own distribution**:
- Computes views-per-day for every video (normalized for age)
- Applies a 1.5× recency multiplier for videos under 7 days (YouTube pushes fresh content harder)
- Marks a video as "viral" only if it's 2+ standard deviations above that channel's mean

This means the trending signal is always contextually meaningful regardless of channel size.
